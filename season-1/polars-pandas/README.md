...
1. **Clone the repository** (if you haven't already):
   ...
   cd resources/season-1/polars-pandas
   ```
...
3. **Launch the notebook**:
   ...
   **Important**: Always run from the `polars-pandas/` directory so paths resolve correctly.
...
_Note: Results vary based on CPU cores, RAM, and disk speed._
...
```bash
# Make sure you're in polars-pandas/ directory
pwd  # Should show .../season-1/polars-pandas
python data/generate_dataset.py
```
...
```bash
# Navigate to the polars-pandas directory
cd /path/to/resources/season-1/polars-pandas
uv run marimo run resource.py
```

The notebook uses paths relative to its location, so it must be run from the `polars-pandas/` directory.
...
4. **Comments**: Explain _why_, not _what_
