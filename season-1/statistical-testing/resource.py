...
def _(mo):
    mo.md(r"""
    Note that random seeds allow you to reproduce the same random pattern for consistent operations in projects and testing
    """)
    return
...
def _(mo):
    mo.md(r"""
    `stats.ttest_ind` performs an independent samples t-test to determine if there's a statistically significant difference between the means of two independent groups (like comparing test scores between two different classes).
    """)
    return
...
def _(mo):
    mo.md(r"""
    The negative t-statistic (-3.760) indicates that the first group has a significantly lower mean than the second group. With a p-value of essentially zero (< 0.001), we can reject the null hypothesis and conclude there is a statistically significant difference between the two groups.
    """)
    return
...
def _(mo):
    mo.md(r"""
    `stats.chi2_contingency` performs a chi-square test of independence to determine if there's a statistically significant association between two categorical variables in a contingency table (like testing if gender is related to product preference).
    """)
    return
...
def _(mo):
    mo.md(r"""
    The chi-square test shows no significant association between the categorical variables being tested. With a p-value of 0.329 (well above the typical α = 0.05 threshold), we fail to reject the null hypothesis of independence. The observed frequencies in the contingency table are not significantly different from what we would expect by chance alone.
    """)
    return
...
def _(mo):
    mo.md(r"""
    The linear regression shows an excellent fit with a strong positive relationship. For every one-unit increase in the predictor variable, the response variable increases by approximately 2.05 units. The r² = 0.942 indicates that 94.2% of the variance in the dependent variable is explained by the linear relationship, demonstrating a very strong predictive model. The p-value of essentially zero confirms this relationship is highly statistically significant.
    """)
    return
...
def _(mo):
    mo.md(r"""
    The t-test shows a highly significant difference between the treatment and control groups (p < 0.001), with the treatment group performing significantly better. The Cohen's d of 1.100 indicates a large effect size (d > 0.8 is considered large), meaning the treatment produced a substantial practical difference beyond just statistical significance.
    """)
    return
